//! test_z80_against: Test the z80 interpreter in `euphrates` against the emulator
...
//! that could lead to different results between `z80sim` and `euphrates`. In my
...
fn generate_instructions<R: Rng>(count: usize, r: &mut R) -> Vec<InstructionSequence> {
    ...
    euphrates_process_instructions!(process_instruction, d, e, n, nn);
...
impl std::fmt::Debug for TestFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> fmt::Result {
        write!(
            ...
            self.mnemonics, self.original_z80, self.sim_z80, self.euphrates_z80,
        )
    }
}
...
/// Run both `z80sim` and `euphrates` on the instruction sequence.
///
