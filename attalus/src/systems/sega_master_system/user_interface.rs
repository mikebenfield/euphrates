...
pub struct UiStatus<R> {
    master_system: Box<MasterSystem<R>>,
    save_directory: Option<PathBuf>,
    recording_status: RecordingStatus<MasterSystemState>,
...
impl<R> UiStatus<R> {
    pub fn master_system(&self) -> &MasterSystem<R> {
        self.master_system.deref()
    }

    pub fn master_system_mut(&mut self) -> &mut MasterSystem<R> {
        self.master_system.deref_mut()
    }

    pub fn master_system_own(self) -> Box<MasterSystem<R>> {
        self.master_system
    }
    ...
    pub fn save_state(&mut self, name: Option<&str>) {
        if let Some(mut path) = self.save_directory.clone() {
            ...
            let state = self.master_system.state();
...
pub trait UiHelper<R> {
    fn frame_update(&mut self, ui: &mut UiStatus<R>) -> Result<Option<PlayerStatus>>;
}

pub struct Ui<R> {
    status: UiStatus<R>,
    helper: Box<UiHelper<R>>,
}
...
impl<R> Ui<R> {
    ...
    pub fn run(&mut self) -> Result<()> {
        self.status.master_system.init()?;
        loop {
            match self.helper.frame_update(&mut self.status)? {
                None => return Ok(()),
                Some(player_status) => {
                    self.status.recording_status.update(player_status);
                    self.status.master_system.run_frame(player_status)?;
                }
            };
        }
    }
