...
pub trait HoldableImpl {
    type Impler: HoldableImpler<Self>;
}

impl<S> Holdable for S
where
    S: HoldableImpl + ?Sized,
{
    #[inline]
    fn holding(&self) -> bool {
        <S as HoldableImpl>::Impler::holding(self)
    }
}

pub trait Inbox: Holdable {
...
impl<S> HoldableImpler<S> for NothingInbox {
    #[inline]
    fn holding(_: &S) -> bool {
        false
    }
